// File: FingerprintOverlayService.kt
package com.panmetro.iptv.globalFingerprintService
...
import com.panmetro.iptv.extensions.generateTextFingerprint
...
class FingerprintOverlayService : Service() {
            ...
            object : EventSourceListener() {
                ...
                override fun onEvent(eventSource: EventSource, id: String?, type: String?, data: String) {
                    try {
                        ...
                        loge(TAG, "Error parsing SSE JSON: ${e.message}")
                    }
                }

                override fun onClosed(eventSource: EventSource) {
                    loge(TAG, "SSE closed by server. Reconnecting in 3s…")
                    mainHandler.postDelayed({ startFingerprintSSE() }, 3_000)
                }

                override fun onFailure(eventSource: EventSource, t: Throwable?, response: Response?) {
                    loge(TAG, "SSE failure: ${t?.message}. Reconnecting in 3s…")
                    eventSource.cancel()
                    mainHandler.postDelayed({ startFingerprintSSE() }, 3_000)
                }
            }
    ...
    private fun updateAllFingerprints(fps: List<Fingerprint>) {
        if (PanmetroApplication.isInForeground) {
            clearAllOverlays()
            return
        }
        ...
        fps.forEach { fp ->
            val intervalMillis = ((fp.intervalSec ?: 5f) * 1_000L).toLong()
            ...
            val fingerprintRunnable = object : Runnable {
                override fun run() {
                    ...
                    val (posX, posY) = if (fp.positionMode == "RANDOM") {
                        val fracX = kotlin.random.Random.nextFloat().coerceIn(0f, 1f)
                        val fracY = kotlin.random.Random.nextFloat().coerceIn(0f, 1f)
